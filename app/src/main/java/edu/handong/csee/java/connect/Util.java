...
//			}
		...
		while (true) {
			...
			int check = 0;
			for (int i = 0; i < dont.size(); i++) {
				if (dont.get(i).x == (int)((double) firstX * 41.7 + 26) && dont.get(i).y == (int)(firstY * 41.6 + 28))
					check = 1;
			}
...
//		System.out.println(firstX + " " + firstY + "        " + secondX + " " + secondY);
	...
	public static void comBlackFirst(ArrayList<Point> vcCom, int[][] result, ArrayList<Point> dont) {
		int i1 = 0, j1 = 0;
		int i2 = 0, j2 = 0;
		int check = 0;
		for (int i = 0; i < 19; i++) {
