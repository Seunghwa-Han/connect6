...
//					System.out.println("num_mine " + num_mine + "  num_empty " + num_empty);
					if ((num_mine == 3) && (num_empty == 1)) {
						for (k = 1; k < 5; k++) {
							fill_2x2(ar, 18 - j - k, i + j + k, vcCom, resultt);
						}
						put_count++;
						if (put_count == 2) {
//							put_count = 0 ;
...
//						put_count = 0 ;
						...
						int ct = 0;
						for (k = 1; k < 6; k++) {
							ct = fill_2x1(ar, 18 - j - k, i + j + k, vcCom, resultt, ct);
							if (ct == 3)
								break;
						}
						put_count++;
